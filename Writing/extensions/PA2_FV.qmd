...
The training procedure for both the YOLO and RF-DETR architectures was carried out under identical computational and hyperparameter conditions to ensure a fair and direct comparative evaluation.
  ...
  - **Cross Validation:** The training procedure involved 5-fold Cross Validation (CV), but with a critical modification: a dedicated 20% portion of the entire dataset was initially isolated as the final, unseen test dataset (Holdout Set). This 20% test set remained identical and unaltered across all training runs and architectures. The remaining 80% of the data was utilized for the actual 5-fold CV process to tune models and determine validation metrics. The retention of this consistent, unseen test dataset was methodologically mandatory, as it formed the basis for establishing a scientific benchmark to fairly compare the models' true generalization capabilities.

## Model Testing
The final testing of the trained models was conducted under the same computational conditions as the training phase. The best weights of the 5 fold cross validation were selected. For evaluation, the separate test splits of both Datasets A and B were used exclusively. This ensured that the model performance metrics were derived from data the models had not previously encountered during the training or validation phases.
...
### Dataset A
The 5-fold cross validation produced results displayed in (@tbl-5cv-dataseta-yolo).

```{python}
#| tbl-cap: "Results of 5-fold Cross Validation YOLO (Dataset A)"
#| label: tbl-5cv-dataseta-yolo
metric_names_yolo_a = [
    "Precision",
    "Recall",
    "mAP50",
...
![Training Results of Best Weights YOLO (Dataset A)](figures/Roboflow/YOLO/results_roboflow_yolo_train.png){#fig-yolo-roboflow-train}

{{< pagebreak >}}

The 2-fold cross validation produced results displayed in (@tbl-5cv-dataseta-rfdetr).

```{python}
#| tbl-cap: "Results of 2-fold Cross Validation RF-DETR (Dataset A)"
#| label: tbl-5cv-dataseta-rfdetr
import numpy as np
import pandas as pd

# 1. Metrik-Namen definieren (in der korrekten COCO-Reihenfolge)
# Wir müssen den Keys eindeutige Zusätze geben (all, small, medium, large etc.),
# da sie im Originaltext identisch hiessen.
metric_names_rfdetr = [
...
# Ich habe die Werte aus deinem Text extrahiert. Listen behalten die Reihenfolge.
...
# axis=0 bedeutet, wir berechnen den Durchschnitt "von oben nach unten" durch die Folds
means_rfdetr = np.mean(all_folds_rfdetr, axis=0)
stds_rfdetr = np.std(all_folds_rfdetr, axis=0)

# 4. Ergebnis schön darstellen
# Wir erstellen ein DataFrame für eine saubere Tabelle
...
![Training Results of Best Weights RF-DETR (Dataset A)](figures/Roboflow/RF-DETR/metrics_plot_roborf_train.png){#fig-rfdetr-roboflow-train}  
...
```{python}
#| tbl-cap: "Results of 5-fold Cross Validation YOLO (Dataset B)"
...
```{python}
#| tbl-cap: "Results of 5-fold Cross Validation RF-DETR (Dataset B)"
...
data = {
    "YOLO": [0.869, 0.799, 0.833, 0.839, 0.589],  # Platzhalter für die Werte
    "RF-DETR": [0.776, 0.759, 0.767, 0.840, 0.590] # Platzhalter für die Werte
}
...
Based on the results of @tbl-dataset-a a visualisation was generated:
...
# Wir verwenden die Metriken als Index und die Modelle als Spalten-Header
data = {
    "YOLO": [0.960, 1.000, 0.980, 0.995, 0.784],  # Platzhalter für die Werte
    "RF-DETR": [1.000, 1.000, 1.000, 1.000, 0.770] # Platzhalter für die Werte
}
...
Based on the results from @tbl-dataset-b a visualisation was generated:
...
**Sample Images from the YOLO Model (RF-DETR does not generate any)**  
