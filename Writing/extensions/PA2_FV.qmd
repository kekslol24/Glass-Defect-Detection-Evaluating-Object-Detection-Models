...
The aggregated data from the validation phase (@tbl-5cv-dataseta-yolo-agg and @tbl-5cv-dataseta-rfdetr-val) indicated that the RF-DETR model performed slightly better than the YOLO model, a finding which was also visually evident in the corresponding graph (@fig-valid-data-a). But because of the high variance the observed higher mean performance of RF-DETR was statistically unreliable. The underlying variance (Std.) of the RF-DETR model is either at the same level as the YOLO model's variance or slightly above it. This constraint must be considered when assessing the RF-DETR's true robustness.

The aggregated data from the validation phase (@tbl-5cv-dataseta-yolo-agg and @tbl-5cv-dataseta-rfdetr-val) indicated a numerically slight superiority for the RF-DETR model over the YOLO model, an observation visually supported by the corresponding graph (@fig-valid-data-a). However, the high variance associated with the RF-DETR's performance rendered its observed higher mean unreliable. The instability confirmed that the minimal numerical advantage achieved by the RF-DETR was not trustworthy compared to the statistical certainty of the YOLO model.
...
- **Performance Comparison:** Although the RF-DETR model achieved a numerically higher mean recall in the final test set (0.842) than the YOLO model (0.634), this metric was not statistically reliable due to the RF-DETR's high standard deviation ($ \pm 0.098$) and the methodological constraint of only two CV runs aswell as the high computation power and extremely long execution time. Therefore the YOLO architecture was ultimately identified as the superior choice for the glass defect application.  

- **Applicaton Benchmarks:** The achieved Recall of 0.634 by YOLO on the complex dataset is insufficient for final deployment. The necessity for the industry to achieve a Recall and Precision closer to 1.000 was confirmed as the mandatory application benchmark, suggesting that further research focused on robustness and data synthesis is required.
