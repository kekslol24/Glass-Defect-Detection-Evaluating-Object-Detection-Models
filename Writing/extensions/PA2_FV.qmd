...
2. **Data management and fine-tuning:** To test the models' generalizability, they were each finetuned to the specific classification task (defect detection) using two discrete datasets (dataset A and dataset B).
...
![Raw example images of the dataset A (Ground Truth)](figures/val_batch0_labels_roboflow.jpg){#fig-roboflow-raw width=40%}
...
data = {
    ...
    "Train Time (Approx.)": ["~ 200min", "~ 1500min", "~ 25 min", "~ 111 min"]
}
